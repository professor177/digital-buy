  ...
  return (
    <form onSubmit={save} className="mb-6 rounded-lg border border-brand/30 bg-panel p-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <Label>Price in BDT (blank = TBD)</Label>
          <input
            className="field"
            inputMode="numeric"
            value={form.priceBdt ?? ""}
        ...
        <div className="space-y-2">
          {games.map((g) => (
            <div key={g.id} className="flex flex-wrap items-center gap-3 rounded-lg border border-line bg-panel px-4 py-3">
              ...
              <span className="text-xs text-fog">{g.priceBdt != null ? bdt(g.priceBdt) : "TBD"}</span>
                ...
                <div className="mt-3 space-y-1.5">
                  ...
                  {pkgs.map((pkg) => (
                    <div key={pkg.id} className="flex flex-wrap items-center gap-3 rounded-md border border-line/70 bg-panel2 px-3 py-2">
                      ...
                      <span className="text-xs text-fog">{pkg.priceBdt != null ? bdt(pkg.priceBdt) : "TBD"}</span>
...
function UbisoftPanel() {
  ...
  if (!rental) return <p className="py-10 text-center text-sm text-fog">No rental record found. Run the catalog seed.</p>;
  ...
  return (
    <form onSubmit={save} className="max-w-2xl rounded-lg border border-line bg-panel p-6">
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <Label>Monthly price in BDT</Label>
          <input
            className="field"
            inputMode="numeric"
